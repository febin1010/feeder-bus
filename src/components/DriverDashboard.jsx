...
const DriverDashboard = () => {
  ...
  const fetchRoutes = async (token) => {
    setLoadingRoutes(true); // Start loading
    try {
      ...
      if (response.ok) {
        ...
        setLoadingRoutes(false); // Stop loading even if there is an error
      }
    } catch (error) {
      console.error('Error fetching routes:', error);
      setLoadingRoutes(false); // Stop loading on error
    }
  };
    ...
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 to-purple-600 p-4">
      ...
      <div className="w-full max-w-4xl bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="flex flex-col">
            ...
            {loadingRoutes ? (
              <select className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-600" disabled>
              ...
              >
                {routes.map((route, index) => (
                  <option key={index} value={`${route.from_location} to ${route.to_location}`}>
