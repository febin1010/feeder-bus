...
const startServer = async () => {
  try {
    ...
    app.post('/api/login', async (req, res) => {
      ...
      try {
        ...
        if (await bcrypt.compare(password, user.password)) {
          const token = jwt.sign({ driver_no: user.driver_no, email: user.name }, JWT_SECRET, { expiresIn: '5h' });
    ...
    app.get('/api/routes', authenticateToken, async (req, res) => {
      try {
        const routesQuery = 'SELECT from_location, to_location FROM routes';
    ...
    app.post('/api/start-trip', authenticateToken, async (req, res) => {
        ...
        const { driver_no,email } = req.user;
      
        // Get the current date in the format you want
        const currentDate = new Date().toISOString().split('T')[0];
      
        try {
          const query = 'INSERT INTO trips (driver_no,driver_name, trip, time, date, bus_no) VALUES (?, ?, ?, ?, ?, ?)';
          const [result] = await db.execute(query, [driver_no,email, trip, time, currentDate, busNumber]); // Include busNumber in the query
